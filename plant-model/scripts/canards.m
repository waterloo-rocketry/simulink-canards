...
rocket_radius = Lr / 2; % tail radius [m]
c1_canard = ((chord_root+chord_tip) /  2) * (rocket_radius^2) * span;
c2_canard = ((chord_root + 2*chord_tip)/3) * rocket_radius * (span^2);
...
Y_canard = rocket_radius + (span/3) * yparcial_canard; %mean aerodynamic chord distance with the radius added
...
Cnalfat_canard = ((4 * N_canard * (span / Lr) ^ 2) / (1 + sqrt(1 + (2 * Lf_canard / (chord_root + chord_tip)) ^ 2))) * (1 + rocket_radius / (span + rocket_radius));
