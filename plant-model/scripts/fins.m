function [fin_pos_x_cp, Cnfdelta, CndNi, fin_CNa, fin_aspectratio, fin_area, fin_midchord_angle, fin_dist_chord_mean, fin_pos_r_chord_mean, fin_leading_edge] = fins(Cr, Ct, fin_height, sweep, pos_aletas, rt, N_fins, Ar, Lr, r0)
...
fin_Cna_0 = ((4 * fin_number * (fin_height / Lr) ^ 2) / (1 + sqrt(1 + (2 * fin_leading_edge / (Cr + Ct)) ^ 2))) * (1 + rt / (fin_height + rt));
fin_CNa = fin_Cna_0 * (1 + r0/(fin_height + r0)); %interference factor
end