function [fin_pos_x_cp, Cnfdelta, CndNi, CNa_fins, fin_aspectratio, fin_area, fin_midchord_angle, fin_dist_chord_mean, fin_pos_r_chord_mean, fin_leading_edge] = fins(Cr, Ct, fin_height, sweep, pos_aletas, rt, N_fins, Ar, Lr, r0)
...
CNa_fins = ((4 * fin_number * (fin_height / Lr) ^ 2) / (1 + sqrt(1 + (2 * fin_leading_edge / (Cr + Ct)) ^ 2))) * (1 + rt / (fin_height + rt));
