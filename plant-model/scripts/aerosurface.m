function [pos_x_cp_subsonic, pos_r_chord_mean, area_planform, aspect_ratio, midchord_angle, fin_factor, pos_x_cp_mach2] = aerosurface(chord_root, chord_tip, height, sweep_angle, pos_x, fin_number, rocket_diameter)
    ...
    % variables
    mach = 2.5; % calculate supersonic parameters for Ma = 2. This hould be done online in the future
    beta = sqrt(mach^2-1);
    ...
    pos_r_chord_mean = rocket_diameter/2 + height_aerodynamic_mean; %mean aerodynamic chord distance with the radius added

    % center of pressure axial
    chord_aerodynamic_mean = 2/3 * (chord_root + chord_tip - (chord_root*chord_tip)/(chord_root+chord_tip));
    pos_x_aerodynamic_mean = height_aerodynamic_mean * tan(sweep_angle);
    % pos_x_cp_subsonic = pos_x - pos_x_tip/3 * (chord_root+2*chord_tip)/(chord_root+chord_tip) - 1/4 * chord_aerodynamic_mean;
    pos_x_cp_subsonic = pos_x - pos_x_aerodynamic_mean - 1/4 * chord_aerodynamic_mean;
    pos_x_cp_mach2 = pos_x - pos_x_aerodynamic_mean - chord_aerodynamic_mean * (aspect_ratio*beta - 0.67) / (2*aspect_ratio*beta - 1);
