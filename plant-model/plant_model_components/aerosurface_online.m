function [C_normal_alpha_1] = aerosurface_online(mach, incidence, height, midchord_angle, area_planform)
    ...
    aspect_ratio = height^2/area_planform;
    ...
    if mach <= subsonic % subsonic
        C_normal_alpha_1 = (2 * pi * height^2 ) / ...
                           (1 + sqrt( 1 + ( beta * aspect_ratio / cos(midchord_angle) )^2 ) );
    ...
    elseif mach > supersonic % supersonic
        K_n = [((gamma+1)*mach^8+(2*gamma^2-7*gamma-5)*mach^6+10*(gamma+1)*mach^4+8)/(6*beta^7); 
             ((gamma+1)*mach^4-4*beta^2)/(4*beta^4);  
              2/beta];
        C_normal_alpha_1 = polyval(K_n, incidence) * area_planform;

    else % transonic
        subsonic_beta = sqrt(abs(1-subsonic^2));
        subsonic_temp1 = sqrt( 1 + ( subsonic_beta * aspect_ratio / cos(midchord_angle) )^2 );
        subsonic_value = (2 * pi * height^2 ) / (1 + subsonic_temp1 );
        subsonic_deriv = 2 * mach * pi * height^6 / ( (area_planform * cos(midchord_angle))^2 * subsonic_temp1 * (1 + subsonic_temp1)^2 );
        
        supersonic_beta = sqrt(abs(1-supersonic^2));
        supersonic_K_n = [((gamma+1)*supersonic^8+(2*gamma^2-7*gamma-5)*supersonic^6+10*(gamma+1)*supersonic^4+8)/(6*supersonic_beta^7); 
                          ((gamma+1)*supersonic^4-4*supersonic_beta^2)/(4*supersonic_beta^4);  
                            2/supersonic_beta];
        supersonic_value = polyval(supersonic_K_n, incidence) * area_planform;
        supersonic_deriv = - 2* area_planform * supersonic / supersonic_beta^3;
