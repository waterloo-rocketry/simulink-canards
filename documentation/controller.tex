...
The model used for controller design is thus simplified to the state \ref{eq:controller-model-state}, using a linear parameter-varying system\footnote{Technically the ``full'' state is projected unto the controllable subspace.}.
...
The concocted state $x_R$ for the roll model is (Equation \ref{eq:controller-model-state})
\begin{align}
...
\begin{align}    
    ...
    \nonumber
\end{align}
where the roll control derivative is $L_\delta (\bar p,  \,  C_L)$, and the roll damping derivative is $L_{\omega_x} (\bar p, \, C_{L \omega})$.
...
The design points $x_s$ of the schedule are characterized by these slowly-varying flight conditions, being the aerodynamic pressure $\bar p$, and the lift coefficient of the canards $C_L$. 
The simplified model is evaluated around these design points using the current state estimate $A \rvert_{\hat x}$, to form the matrix $A$.
