...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.45\linewidth]{images-finn/model-coordinates-rocket.png}
...
This unit constraint has to be enforced in the filter process, e.g. by norming $q$ after integration \cite{sola2017}, or by Baumgarte stabilization (implemented is the former).
...
\subsubsection{Airfoil theory and lift coefficient}
...
Protruding geometries of the rocket body, such as the canards, have shock waves attached to them if the flow around them is supersonic.
At higher speeds, these shock waves angle more and more backwards. 
The characteristic Mach angle is defined as non-zero only for $\mathrm{Ma} \geq 1$, as \cite{stengel2004}
\begin{equation}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=0.9\textwidth]{images-finn/airfoil_angles.png}
        \caption{Lift coefficient and Mach cone angle vs. Mach number, for 60° sweep angle}
        \label{fig:airfoil-angles}
    \end{subfigure}
    \begin{subfigure}{0.49\textwidth}
        \includegraphics[width=0.9\textwidth]{images-finn/airfoil_sweeps.png}
...
\subsubsection{Canard forces and roll moments}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.5\linewidth]{images-finn/actuator_cad.png}
...
The gear ration is assumed to be $\beta_\delta = 1$ from now on, following equations need to adjusted if that shoudl change. 
...
\end{align}
As input to the system serves the commanded canard angle $\delta_u$, and all states are available as outputs du to the state estimation.
...
The 1st order actuator dynamics are accounted for with the $\delta$ state, and are characterized by the time constant $\alpha$ (the value should be determined by experimental system identification).
