In this chapter, the rocket flight is modelled, and a control loop consisting of state estimation and a controller is designed using this model.
...
The state $x$ describes a collection of variables, which describe the dynamics of the system in a collection of first order differential equations.
\footnote{For example: a mass-spring-damper system has two state variables, position and velocity. These describe the energy stores of the system, potential of the deformed spring and kinetic of the moving mass.}
These states do not have to have a physical meaning \footnote{For example, one could describe a system with its input-output relationship, without assigning a meaning to the variables of the differential equations.}, but they will in this case as they describe the system well.
...
This mostly encompasses measurements taken from the system by sensors, or the variables of interest for a control loop. 
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.4\linewidth]{images-finn/model-coordinates-rocket.png}
...
\begin{figure}[ht]
    ...
    \input{images-finn/model_coordinate-systems}}
...
%     \centering
%     \resizebox{0.36\textwidth}{!}{
%     \input{images-finn/model_coordinates2}}
...
\begin{equation}
    ...
    \end{bmatrix}
\end{equation}
which consists of four variables, often names as the real part $q_w$, and the vector part $q_v$.\footnote{Formally, the quaternion is a four-dimensional complex number, instead of $a+\mathrm{i}b$ it is $q_w + \mathrm{i} q_x + \mathrm{j} q_y + \mathrm{k} q_z$, where all variable are orthogonal and $\mathrm{i}^2 = \mathrm{j}^2 = \mathrm{k}^2 = \mathrm{ijk} = -1$. This will not really be relevant for the mechanics, but explains some weirdness of the calculations.}
The attitude quaternion is normed to $|q| = q^T q =  1$ to describe rotations.
This unit constraint has to be enforced in the filter process, e.g. by norming $q$ after integration \cite{sola2017}.
...
The inverse of a unit quaternion is simply \cite{stevens2015}
\begin{align}
    q^{-1} = \begin{bmatrix}
        q_w \\ q_v
...
The time derivative of the quaternion attitude, with angular rates $\omega$ described in the body-fixed frame, is \cite{zipfel2007, sola2017, stevens2015}
\begin{align}
    \dot q \;
    &= \; \frac{1}{2} \tilde \Omega \cdot q \;
    = \; \frac{1}{2} \tilde Q \cdot 
    \begin{bmatrix}
        0 \\ \omega
    \end{bmatrix}
    & 
...
\begin{align}
    dq(t, dt) &= \begin{bmatrix} \cos(\frac{d\varphi}{2}) \\ dn \sin(\frac{d\varphi}{2})
    \end{bmatrix}
...
For initialization, the quaternion is set according to Equation \ref{eq:rotation-theorem}, where $\varphi$ the launch angle, and $n$ indicates the launch orientation (pitch or yaw) \footnote{Initialization with Euler angles is also possible\cite[p. 126]{zipfel2007}, but too much effort to write down here.}.
A typical initialization would be $\varphi = $ launch angle, and $\bm d = [\begin{smallmatrix} 0 & 1 & 0 \end{smallmatrix}]^T$, i.e. the rocket is pitched downrange, and both yaw and roll are defined zero.
...
\begin{align}
    \dot v = \mathcal{F}/m - 2 \, \omega \times v + S g^G
...
Note: gravity is thus not a part of the force $\mathcal{F}$.
...
The Inertial Measurement Units (IMUs) which house all sensors\footnote{Simplified here: an IMU is defined as Gyroscope + Accelerometer, but Magnetometer and Barometer are on the same board/housing.}, are not mounted at the center of gravity of the rocket body, and the sensor frame is not aligned to the body frame.
...
\begin{equation}
    M_E^G = S^T ( B_{SI}^{-1} (S_M M - b_{HI} ) \label{eq:meas-mag}
\end{equation}

\subsubsection{Pressure - Barometer}
The barometer measures the local air pressure with the sensor offset bias as
\begin{equation}
    P = p + b_P
\end{equation}
...
Somewhere in the gearing between servo actuator and canard surface, there is a rotational encoder on the shaft.
The encoder measures the canard angle directly, and the resulting encoder signal $\delta_e$ is 
\begin{align}
    \delta_e = \delta
\end{align}
...
However, the nominal trajectory goes well outside operational limits of most (or all) commercial GPS receivers (being |a| < 4.5g, alt < 18km, |v| < 515m/s), the altitude and velocity limits are imposed by the US International Traffic in Arms Regulations.
...
To predict the barometer measurements and the aerodynamic effects, it is important to determine these variables accurately.
The air at altitude $l$ is modelled according to the US standard atmosphere \cite{stengel2004} as
\begin{align}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=0.9\textwidth]{images-finn/model_atmosphere-pressure.png}
        \caption{Pressure}
        \label{fig:atmos-pressure}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=0.9\textwidth]{images-finn/model_atmosphere-temperature.png}
        \caption{Temperature}
        \label{fig:atmos-temperature}
    \end{subfigure}
    \begin{subfigure}{0.32\textwidth}
        \includegraphics[width=0.9\textwidth]{images-finn/model_atmosphere-density.png}
        \caption{Density}
...
\end{equation}
which is the predominant factor in the aerodynamic forces and moments.
As the local speed of sounds varies with altitude, the Mach number is $\mathrm{Ma} = V_A / c $.
The Mach number has influence on the aerodynamic coefficients, and the location of the center of pressure.

\subsubsection{Angle of attack \& sideslip}
The attitude of the rocket relative to the oncoming airstream is defined with the \textit{angle of attack} $\alpha$ and the \textit{angle of sideslip} $\beta$.
...
\begin{align}
    |v_x| & \geq 0  & |v_x| &\leq 0\nonumber \\
        \alpha &= \arctan(v_z/v_x) & \alpha &= \pi - \arctan(v_z/v_x) \\
        \beta &= \arctan(v_y/v_x) &  \beta &= \pi - \arctan(v_y/v_x)
\end{align}

\begin{figure}[ht]
    \centering
    \resizebox{0.25\textwidth}{!}{
    \input{images-finn/model_coordinates_aero}}
...
The rocket dynamics have external forces and torques acting on the body, whcih are described here.
...
The model could also be used not as a filter, but e.g. in a flight simulation, where the forces would be used.
The external torques are still important however, as they couple some states to the angular velocity measurements.

\subsubsection{Aerodynamic forces and moments}
...
Any aerodynamic force for each component is defined as 
\begin{align}
    \mathcal{F}^{\,i} &= \bar p \, C^i_{...} A 
\end{align}
where the aerodynamic coefficient (lift $C_L$, normal $C_N$, drag $C_D$, roll $C_l$) and reference area $A$ depend on the geometry of the component, and the lift coefficient encompasses information about the angle of attack. 
...
Here $^i$ indicates an arbitrary component (it is not an exponent).
...
\end{align}
with the coefficients listed in Table \ref{tab:aero-coeff}.
...
Symbol & Value & Name \\
\hline
$C^\text{can}_{L_\delta}$ or $C_L$ & & Lift coefficient, canards \\
...
\subsubsection{Aerodynamics of the canards and roll moments}
The coefficient of lift is approximated using thin airfoil theory \cite{stengel2004}.
The canards have a low aspect ratio of about $0.5 \lesssim A\!\!R \lesssim 1$, and thin airfoil theory thus predicts a lift coefficient for each canard of \cite{stengel2004}
\begin{equation}
    C_L^\text{can} = \frac{\pi}{2} A\!\!R \, \delta \approx 1.5 \, \delta
\end{equation}
for small angles of attack $\delta < 40^\circ$ of the canard surfaces.
...
\begin{figure}[ht]
    ...
    \input{images-finn/model_canard-angle}}
...
\begin{align}
    ...
    \\ 0 \\ 0 \end{bmatrix}
\end{align}
where $A^\text{can}$ is the total area of all canards and $d^\text{can}_y$ is the distance between the rocket roll axis $x^B$ and the center of pressure of one of the canards.
...
\end{align}
which simplifies notation of the roll dynamics as shown in Section \ref{sec:model-roll-dynamics}.
...
\begin{align}
    ...
    \begin{bmatrix} 
    0 \\ 
    \bar p \, C_{N_\alpha} \alpha A^\text{ref} \\ 
    \bar p \, C_{N_\beta} \beta A^\text{ref}
    \end{bmatrix}
    & 
...
This leaves an axial propulsion force as some function of the engine performance $F_\text{engine}$, so
\begin{align}
...
This effect only acts during the first few fractions of a second, and will therefore be neglected for the flight.
...
The control input vector is the commanded canard angle to the actuator $\delta_u$ and, as stated in the previous paragraph, the  accelerometer data $A = \begin{bmatrix} A_x & A_y & A_z \end{bmatrix}^T$, which may also be stacked.
...
If multiple IMUs are used, the measurements are stacked in the input vector as $u = [\begin{smallmatrix} \delta_u & A_1 & A_2 & A_3 \end{smallmatrix}]$.
...
This is because the actuating canards can only produce a roll torque, i.e. a moment about the $x$-axis, but cannot directly influence other states, e.g. pitch and yaw rates.
